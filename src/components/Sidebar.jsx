    ...
    <div className="w-64 min-h-screen bg-white border-r border-gray-200 flex flex-col">
      ...
      <div className="p-6 border-b border-gray-100">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h2 className="font-semibold text-gray-900 tracking-tight">Papi Coffee</h2>
      ...
      <nav className="flex-1 p-4 space-y-1">
        {menuItems.map((item) => (
          ...
          >
            {({ isActive }) => (
              <>
                <item.icon className={`w-5 h-5 transition-colors ${isActive ? "text-amber-600" : "text-gray-400 group-hover:text-gray-600"}`} />
      ...
      <div className="p-4 border-t border-gray-100">
        <div className="flex items-center gap-3 px-3 py-2.5 rounded-lg hover:bg-gray-50 transition-all cursor-pointer">
          ...
          <div className="flex-1 min-w-0">
            <p className="text-sm font-medium text-gray-900 truncate">Papi Admin</p>
            <p className="text-xs text-gray-500 truncate">admin@papicoffee.com</p>
          </div>
        </div>
      </div>
    </div>
