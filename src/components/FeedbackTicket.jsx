...
export default function FeedbackTicket({ feedback }) {
  return (
    <div className="bg-white rounded-xl border border-coffee-200 p-5 hover:shadow-md transition-all">
          ...
          <div>
            ...
            <div className="flex items-center gap-1 mt-1">
              {Array.from({ length: 5 }).map((_, i) => (
                <Star key={i} className={`w-3 h-3 ${i < feedback.rating ? "fill-amber-400 text-amber-400" : "fill-coffee-200 text-coffee-200"}`} />
              ))}
            </div>
          </div>
        </div>
      ...
      <p className="text-coffee-700 text-sm mb-3">{feedback.message}</p>
