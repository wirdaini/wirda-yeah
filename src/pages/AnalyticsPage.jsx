...
export default function AnalyticsPage() {
  ...
  const segmentData = Object.entries(segmentSales).map(([name, value]) => ({ name, value }));
  ...
  const newMembers = membersData.filter((m) => m.segmen === "Pelanggan Baru").length;

  return (
    <div className="p-6 space-y-6">
      <PageHeader title="Analitik" subtitle="Dashboard analytics dan insights bisnis" breadcrumb="Analitik" />

      <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
        <StatCard label="Total Revenue" value={`Rp ${(totalRevenue / 1000000).toFixed(1)}jt`} icon={DollarSign} trend="18% vs minggu lalu" trendUp={true} color="green" />
        <StatCard label="Total Orders" value={totalOrders} icon={ShoppingBag} trend="12% vs minggu lalu" trendUp={true} color="blue" />
        <StatCard label="Avg Order Value" value={`Rp ${(avgOrderValue / 1000).toFixed(0)}K`} icon={null} trend="5% vs minggu lalu" trendUp={true} color="amber" />
        <StatCard label="New Members" value={newMembers} icon={Users} trend="22% vs minggu lalu" trendUp={true} color="purple" />
      </div>
        ...
        <Card>
          ...
          <ResponsiveContainer width="100%" height={300}>
            <LineChart data={weeklyTrend}>
              ...
              <Legend />
              <Line yAxisId="left" type="monotone" dataKey="revenue" stroke="#10b981" strokeWidth={3} dot={{ fill: "#10b981", r: 5 }} name="Revenue" />
              <Line yAxisId="right" type="monotone" dataKey="newMembers" stroke="#8b5cf6" strokeWidth={3} dot={{ fill: "#8b5cf6", r: 5 }} name="New Members" />
            </LineChart>
          </ResponsiveContainer>
        </Card>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <Card>
          ...
          <ResponsiveContainer width="100%" height={300}>
            <PieChart>
              <Pie data={topProducts} cx="50%" cy="50%" labelLine={false} label={({ name, percent }) => `${name}: ${(percent * 100).toFixed(0)}%`} outerRadius={100} dataKey="value">
        ...
        <Card>
          ...
          <div className="space-y-4">
            {segmentData.sort((a, b) => b.value - a.value).map((segment, index) => (
              <div key={segment.name}>
                <div className="flex items-center justify-between mb-2">
                  <Badge type="info">{segment.name}</Badge>
                  <span className="text-sm font-semibold text-gray-900">Rp {(segment.value / 1000000).toFixed(1)}jt</span>
                </div>
                <div className="w-full h-3 bg-gray-100 rounded-full overflow-hidden">
                  <div className="h-full rounded-full transition-all" style={{ width: `${(segment.value / Math.max(...segmentData.map(s => s.value))) * 100}%`, backgroundColor: COLORS[index % COLORS.length] }} />
                </div>
              </div>
            ))}
          </div>
