          ...
          <AccordionItem value="statistics" className="border-0">
            <AccordionTrigger className="px-6 py-4 hover:no-underline hover:bg-gray-50/50">
              <div className="flex items-center gap-2">
                ...
                <span className="font-semibold text-gray-900">Ringkasan Statistik Feedback</span>
                ...
                <div className="bg-gradient-to-br from-amber-50 to-amber-100/50 p-4 rounded-lg">
                  ...
                  <div className="flex items-center gap-1 mt-1">
                    {Array.from({ length: 5 }).map((_, i) => (
                      <Star key={i} className={`w-3 h-3 ${i < Math.round(Number(avgRating)) ? "fill-amber-500 text-amber-500" : "fill-gray-300 text-gray-300"}`} />
                    ))}
                  </div>
                </div>
          ...
          <div className="text-center mb-4">
            ...
            <div className="flex items-center justify-center gap-1 mb-1">
              {Array.from({ length: 5 }).map((_, i) => (
                <Star
                  key={i}
                  className={`w-5 h-5 ${i < Math.round(Number(avgRating)) ? "fill-amber-400 text-amber-400" : "fill-gray-200 text-gray-200"}`}
                />
              ))}
            </div>
